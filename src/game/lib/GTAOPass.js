...
class GTAOPass extends Pass {
  constructor(
    ...
    this.gtaoMaterial.defines.PERSPECTIVE_CAMERA = this.camera
      .isPerspectiveCamera
      ? 1
      : 0;
  ...
  updateGtaoMaterial(parameters) {
    ...
    if (parameters.distanceExponent !== undefined) {
      this.gtaoMaterial.uniforms.distanceExponent.value =
    ...
    if (parameters.distanceFallOff !== undefined) {
      this.gtaoMaterial.uniforms.distanceFallOff.value =
    ...
    if (
      ...
      (parameters.screenSpaceRadius ? 1 : 0) !==
        this.gtaoMaterial.defines.SCREEN_SPACE_RADIUS
    ) {
      this.gtaoMaterial.defines.SCREEN_SPACE_RADIUS =
        parameters.screenSpaceRadius ? 1 : 0;
      this.gtaoMaterial.needsUpdate = true;
  ...
  updatePdMaterial(parameters) {
    ...
    if (
      parameters.samples !== undefined &&
      parameters.samples !== this.pdSamples
    ) {
  ...
  render(renderer, writeBuffer, readBuffer /*, deltaTime, maskActive */) {
    ...
    switch (this.output) {
      ...
      case GTAOPass.OUTPUT.Default:
        ...
        this.blendMaterial.uniforms.tDiffuse.value =
          this.pdRenderTarget.texture;
        this.renderPass(
          renderer,
          this.blendMaterial,
          this.renderToScreen ? null : writeBuffer
